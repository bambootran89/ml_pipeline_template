# Hướng dẫn sử dụng ConfigGenerator với Advanced Pipeline Patterns

## Tổng quan

`ConfigGenerator` đã được nâng cấp để hỗ trợ tự động generate các config eval/serve từ training pipeline có chứa **nested sub-pipelines** và **conditional branches**.

### Các loại pipeline được hỗ trợ:
- ✅ Standard flat pipelines (`standard_train.yaml`)
- ✅ **Nested sub-pipelines (`nested_suppipeline.yaml`)**
- ✅ **Conditional branches (`conditional_branch.yaml`)** ← MỚI
...
### 4. **Conditional Branch Transformation**

Branch steps chứa conditional model selection được transform tương tự:
...
- id: model_selection
  ...
  if_true:
    ...
    type: evaluator            # ← Transformed
    wiring:
      inputs:
        model: fitted_train_tft  # ← Load from MLflow
  ...
  if_false:
    ...
    wiring:
      inputs:
        model: fitted_train_xgb  # ← Load from MLflow
        features: preprocessed_data
        targets: target_data
```
...
**Key Points:**
- Branch condition giữ nguyên trong cả eval và serve modes
- Model producers trong branches (trainers/framework_models) → evaluators (eval) hoặc inference (serve)
- Không generate separate evaluators/inference cho models trong branches
- MLflow loader load tất cả models từ cả 2 branches
...
### Conditional Branch (Adaptive Model Selection)
```bash
# 1. Generate configs
python -m mlproject.src.pipeline.dag_run generate \
...
# 2. Chạy eval (model được chọn dựa trên data_size)
python -m mlproject.src.pipeline.dag_run eval \
    -e mlproject/configs/experiments/etth3.yaml \
    -p mlproject/configs/generated/conditional_branch_eval.yaml \
    -a latest

# 3. Chạy serve (runtime model selection)
python -m mlproject.src.pipeline.dag_run serve \
    -e mlproject/configs/experiments/etth3.yaml \
...
**Lưu ý:** Conditional branch cho phép select model dynamically dựa trên data characteristics:
- Nếu `data_size > 100`: sử dụng TFT model (complex, deep learning)
- Nếu `data_size ≤ 100`: sử dụng XGBoost model (lightweight, faster)

Cả hai models đều được load từ MLflow, và runtime sẽ evaluate condition để chọn model phù hợp.
...
### v3.0 (Current)
- ✅ Added conditional branch support
- ✅ Branch transformation: trainers → evaluators (eval) / inference (serve)
- ✅ Fixed top-level preprocessor handling in all pipeline types
- ✅ Support for adaptive model selection based on runtime conditions
